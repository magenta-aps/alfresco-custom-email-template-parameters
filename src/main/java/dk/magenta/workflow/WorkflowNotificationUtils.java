 ...
 * @since 4.0
 */
public class WorkflowNotificationUtils extends org.alfresco.repo.workflow.WorkflowNotificationUtils
{
    ...
    /** Added by Magenta */
    public static final String ARG_WF_ASSIGNEE = "workflowAssignee";
    public static final String ARG_WF_INITIATOR = "workflowInitiator";
    public static final String ARG_WF_CURRENT_TIME = "workflowCurrentTime";
            ...
            boolean pooled)
    {
        ...
        Map<String, Serializable>templateArgs = new HashMap<String, Serializable>(9);
        ...
        String assignee = assignedAuthorites[0];
        if (personService.personExists(assignee)) {
            assignee = getUserRealName(assignee);
        }
        else
            assignee = "Medlemmer af gruppen \"" + assignee + "\"";
        templateArgs.put(ARG_WF_ASSIGNEE, assignee);

        // Get the workflow task
        String initiator = nodeService.getProperty(workflowPackage, ContentModel.PROP_CREATOR).toString();
        templateArgs.put(ARG_WF_INITIATOR, getUserRealName(initiator));

        String timeStamp = new SimpleDateFormat("EEEE, d. MMMM, yyyy, HH:mm").format(Calendar.getInstance().getTime());
        templateArgs.put(ARG_WF_CURRENT_TIME, timeStamp);
    ...
    private String getUserRealName(String username)
    {
            ...
            String initiatorFirstName = nodeService.getProperty(personRef, ContentModel.PROP_FIRSTNAME).toString();
            String initiatorLastName = nodeService.getProperty(personRef, ContentModel.PROP_LASTNAME).toString();
            String fullName;
            if (initiatorLastName.isEmpty())
                fullName = initiatorFirstName;
            else
                fullName = initiatorFirstName + " " + initiatorLastName;
            return fullName;
    }
